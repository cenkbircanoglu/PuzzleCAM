...
class Classifier_For_Puzzle(Classifier):
    ...
    def forward(self, x, num_pieces=1, level=-1):
        ...
        for l in range(len(layers)):
            l += 1
            if l >= level:
                output_dic["stage%d" % l] = merge_features(
                    output_dic["stage%d" % l], num_pieces, batch_size
                )

        if level is not None:
            output_dic["merged_logits"] = self.global_average_pooling_2d(
                output_dic["stage6"]
            )
...
class AffinityNet(Backbone):
    def __init__(self, model_name, path_index=None):
        ...
        if "50" in model_name or "resnet101" == model_name or "resnet152" == model_name:
