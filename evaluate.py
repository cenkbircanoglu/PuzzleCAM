...
parser.add_argument(
    "--experiment_name",
    default="resnet50@seed=0@nesterov@train@bg=0.20@scale=0.5,1.0,1.5,2.0@png",
    type=str,
)
...
def compare(start, step, TP, P, T, name_list, th):
    for idx in range(start, len(name_list), step):
        name = name_list[idx]

        if os.path.isfile(predict_folder + name + ".npy"):
            predict_dict = np.load(
                os.path.join(predict_folder, name + ".npy"), allow_pickle=True
            ).item()

            if "hr_cam" in predict_dict.keys():
                cams = predict_dict["hr_cam"]
                cams = np.pad(
                    cams, ((1, 0), (0, 0), (0, 0)), mode="constant", constant_values=th
                )
...
def do_python_eval(predict_folder, gt_folder, name_list, num_cls=21, th=None):
    TP = []
    P = []
    T = []
    for i in range(num_cls):
    ...
    for i in range(1):
        p = multiprocessing.Process(
            target=compare, args=(i, 8, TP, P, T, name_list, th)
        )
        p.start()
        p_list.append(p)
    for p in p_list:
...
if __name__ == "__main__":
    ...
    if args.mode == "png":
        loglist = do_python_eval(
            args.predict_dir, args.gt_dir, name_list, 21, th=args.threshold
        )
        print(
            "mIoU={:.3f}%, FP={:.4f}, FN={:.4f}".format(
                loglist["mIoU"], loglist["fp_all"], loglist["fn_all"]
            )
        )
        ...
        for th in th_list:
            loglist = do_python_eval(
                args.predict_dir, args.gt_dir, name_list, 21, th=th
            )
        ...
        print(
            "Best Th={:.2f}, mIoU={:.3f}%, FP={:.4f}".format(
                best_th, best_mIoU, best_FP
            )
        )
        print(
            "Over Th={:.2f}, mIoU={:.3f}%, FP={:.4f}".format(
                over_th, over_mIoU, over_FP
            )
        )
        print(
            "Under Th={:.2f}, mIoU={:.3f}%, FP={:.4f}".format(
                under_th, under_mIoU, under_FP
            )
        )
            ...
            for th in th_list:
                ...
                print(
                    "Th={:.2f}, mIoU={:.3f}%, FP={:.4f}, FN={:.4f}".format(
                        th, loglist["mIoU"], loglist["fp_all"], loglist["fn_all"]
                    )
                )
