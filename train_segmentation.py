...
if __name__ == "__main__":
    ...
    train_dataset = VOC_Dataset_For_WSSS(
        args.data_dir, "train_aug", pred_dir, train_transform
    )
    ...
    save_model_fn_for_backup = lambda: save_model(
        model,
        model_path.replace(".pth", f"_backup.pth"),
                ...
                if step == 0:
                    for b in range(8):
                        ...
                        pred_mask = decode_from_colormap(
                            pred_mask, train_dataset.colors
                        )
                        pred_mask = cv2.resize(
                            pred_mask, (w, h), interpolation=cv2.INTER_NEAREST
                        )

                        image = cv2.addWeighted(image, 0.5, pred_mask, 0.5, 0)[
                            ..., ::-1
                        ]
    ...
    for iteration in range(max_iteration):
        ...
        ###############################################################################
        if "Seg" in args.architecture:
            labels = resize_for_tensors(
                labels.type(torch.FloatTensor).unsqueeze(1),
                logits.size()[2:],
                "nearest",
                None,
            )[:, 0, :, :]
